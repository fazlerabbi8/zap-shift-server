...
// connect lazily — don't block route registration on this
...
app.get("/parcels", async (req, res) => {
  ...
  if (penddingStatus) query.penddingStatus = penddingStatus;
...
app.patch("/parcels/:id/status", async (req, res) => {
  ...
  const updatedDoc = { $set: { penddingStatus: penddingStatus } };
...
app.patch("/parcels/:id", async (req, res) => {
  ...
  const riderQuery = { _id: new ObjectId(riderId) };
  const riderUpdateDoc = { $set: { workStatus: "in-delivary" } };
    ...
    line_items: [
      {
        price_data: {
          currency: "usd",
          unit_amount: amount,
          product_data: { name: parcelInfo.parcelName },
        },
        quantity: 1,
      },
    ],
